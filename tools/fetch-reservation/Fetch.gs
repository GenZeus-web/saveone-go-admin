...
var F = {
  // ชื่อช่องบนหน้ารายงาน (สำรวจมาแล้ว)
...
function isDry_(branch) {
  return DRY_RUN || !!(SHEETS[branch] && SHEETS[branch].dryRun);
}
...
var SHEETS = {
  BG: {
    ...
    food: /^G[A-T]$/,
    car: /^G[W-Z]$/,
    sheetFood: "Report SaveOne Go",
    sheetCar: "Report-Car Boot",
  },
  BN: {
    name: "บางนา",
    ...
    food: /^[A-J]$/,
    car: /^[U-Z]$/,
    sheetFood: "Report Bangna",
    sheetCar: "Bangna - Car Boot", // ชื่อเดา ใช้สำรองเท่านั้น
    gidFood: 1594662584,
    gidCar: 299461870, // จากลิงก์ที่เจ้าของส่งมา — เปลี่ยนชื่อแท็บก็ยังเจอ
...
// ── จุดเริ่ม (กดรันเอง) ──
function fetchBG() {
  fetchBranch_("BG", null);
}
function fetchBN() {
  fetchBranch_("BN", null);
}
...
function setupTriggersBG() {
  setupTriggers_("BG");
}
function setupTriggersBN() {
  setupTriggers_("BN");
}

/** ฟังก์ชันที่ตัวตั้งเวลาเรียก — ต้องไม่รับพารามิเตอร์ */
function scheduledBG() {
  fetchBranch_("BG", null);
}
function scheduledBN() {
  fetchBranch_("BN", null);
}
...
function setupTriggers_(branch) {
  ...
  if (isDry_(branch)) {
    Logger.log(
      "⚠️ ตอนนี้สาขา " +
        branch +
        " อยู่โหมดซ้อม — ตั้งเวลาไปก็จะไม่เขียนอะไรลงชีต",
    );
    Logger.log(
      "   ตั้งได้ แต่อย่าลืมลบ dryRun: true ออกจาก SHEETS." +
        branch +
        " ตอนพร้อมใช้จริง\n",
    );
  }
  ...
  if (removed)
    Logger.log("ลบตัวตั้งเวลาเดิมของ " + handler + " ออก " + removed + " ตัว");
  ...
  Logger.log(
    "เวลาที่รันจริงอาจคลาดจากนี้ได้ถึง 1 ชม. (ข้อจำกัดของ Apps Script)",
  );
}
...
function projectTriggers_(handler) {
  try {
    ...
    Logger.log(
      "⚠️ โปรเจกต์นี้ไม่มีสิทธิ์จัดการทริกเกอร์จากโค้ด (script.scriptapp) — ไม่ได้แตะทริกเกอร์ใดๆ",
    );
    Logger.log(
      "   ตั้งเองได้เลย ไม่ต้องแก้ manifest: แถบซ้าย ⏰ ทริกเกอร์ → + เพิ่มทริกเกอร์ ทำ " +
        RUN_HOURS.length +
        " รอบ",
    );
    Logger.log(
      "   ฟังก์ชัน " +
        h +
        " · Head · ตามเวลา · ตัวจับเวลาตามวัน · ช่วง " +
        RUN_HOURS.map(function (x) {
          return x + ".00–" + (x + 1) + ".00";
        }).join(" / "),
    );
    return null;
...
function listTriggers() {
  ...
  if (!ts) return;
  if (!ts.length) {
    Logger.log("ยังไม่มีตัวตั้งเวลา");
    return;
  }
...
function removeTriggers() {
  var mine = { scheduledBG: 1, scheduledBN: 1 },
    n = 0,
    kept = [];
  var ts = projectTriggers_();
  if (!ts) return;
 ...
 */
function showSheetHeadersBG() {
  showHeaders_("BG");
}
function showSheetHeadersBN() {
  showHeaders_("BN");
}
...
function showHeaders_(branch) {
  ...
  if (!ss) {
    Logger.log(
      "❌ ไม่ได้ตั้ง SG_SHEET_" + branch + " และสคริปต์นี้ก็ไม่ได้ผูกกับชีตไหน",
    );
    Logger.log(
      "   ค่าคือ id ของไฟล์ชีต — ดูจาก URL: docs.google.com/spreadsheets/d/<ตรงนี้>/edit",
    );
    return;
  }
  ...
  [
    [cfg.sheetFood, cfg.gidFood],
    [cfg.sheetCar, cfg.gidCar],
  ].forEach(function (t) {
    var sh = sheetOf_(ss, t[0], t[1]);
    Logger.log("\n──── " + (sh ? sh.getName() : t[0]) + " ────");
    if (!sh) {
      Logger.log("❌ ไม่เจอชีตนี้");
      return;
    }
...
function fetchBranch_(branch, when) {
  var cfg = SHEETS[branch];
  if (!cfg) {
    Logger.log("❌ ไม่รู้จักสาขา " + branch);
    return;
  }
  if (!cfg.report) {
    Logger.log("❌ ยังไม่รู้ URL หน้ารายงานของสาขา " + branch);
    Logger.log(
      "   → เปิดหน้ารายงานในเบราว์เซอร์ ก๊อป URL มาเติมที่ SHEETS." +
        branch +
        ".base / .report",
    );
    return;
  ...
  if (d.getHours() < 18 && sameDay_(d, new Date())) {
    Logger.log(
      "⚠️ ตอนนี้ยังไม่ถึง 18:00 — วอล์กอินอาจยังเข้าไม่ครบ ตัวเลขจะต่ำกว่าจริง",
    );
  }

  var P = PropertiesService.getScriptProperties();
  var user = P.getProperty("SG_USER_" + branch),
    pass = P.getProperty("SG_PASS_" + branch);
  if (!user || !pass) {
    Logger.log("❌ ยังไม่ได้ตั้ง SG_USER_" + branch + " / SG_PASS_" + branch);
    return;
  }
  ...
  var r1 = UrlFetchApp.fetch(REPORT, {
    headers: { Cookie: cookie },
    muteHttpExceptions: true,
  });
  if (r1.getResponseCode() !== 200) {
    Logger.log("❌ เปิดหน้ารายงานไม่ได้ HTTP " + r1.getResponseCode());
    return;
  }
  ...
  if (r2.getResponseCode() !== 200) {
    Logger.log("❌ ค้นหาไม่สำเร็จ HTTP " + r2.getResponseCode());
    return;
  }
  ...
  if (code !== 200 || size < 5000) {
    Logger.log(
      "❌ ไม่ได้ไฟล์ที่คาดไว้ (เล็กเกินไป — น่าจะได้หน้า HTML กลับมาแทนไฟล์)",
    );
    Logger.log(
      "   ตัวอย่างเนื้อหา: " +
        r3.getContentText().slice(0, 300).replace(/\s+/g, " "),
    );
    return;
  }
  ...
  if (!rows.length) {
    Logger.log("❌ แกะไฟล์แล้วไม่เจอข้อมูล");
    return;
  }
...
function writeSheet_(branch, dateStr, z) {
  ...
  if (isDry_(branch)) {
    ["st", "car"].forEach(function (k) {
      var b = z[k],
        nm = k === "st" ? cfg.sheetFood : cfg.sheetCar;
      Logger.log(nm + " ←");
      Logger.log(
        "   Online  " +
          b.online.rai +
    ...
    Logger.log(
      "   ถ้าตัวเลขถูกต้องแล้ว ลบ dryRun: true ออกจาก SHEETS." +
        branch +
        " (หรือ DRY_RUN เป็น false)",
    );
    return;
  }
  ...
  if (id) {
    ...
    if (!ss) {
      Logger.log(
        "❌ ไม่ได้ตั้ง SG_SHEET_" +
          branch +
          " และสคริปต์นี้ก็ไม่ได้ผูกกับชีตไหน",
      );
      Logger.log(
        "   → ใส่ id ของไฟล์ชีตใน Script Properties (ดูจาก URL ของชีต)",
      );
      return;
    }
    Logger.log(
      "ℹ️ ไม่ได้ตั้ง SG_SHEET_" +
        branch +
        " — ใช้ชีตที่สคริปต์ผูกอยู่: " +
        ss.getName(),
    );
  }
  [
    [cfg.sheetFood, cfg.gidFood, z.st],
    [cfg.sheetCar, cfg.gidCar, z.car],
  ].forEach(function (t) {
    var sh = sheetOf_(ss, t[0], t[1]);
    if (!sh) {
      Logger.log(
        '❌ ไม่เจอชีต "' + t[0] + '"' + (t[1] ? " (gid " + t[1] + ")" : ""),
      );
      return;
    }
...
function upsert_(sh, dateStr, c, branchName, label) {
  ...
  var col = {};
  head.forEach(function (h, i) {
    col[String(h).trim()] = i + 1;
  });

  // ช่องที่สคริปต์เขียน → ค่าที่จะใส่
  var owned = [
    ["Online_Rai", c.online.rai],
    ["Online_Lock", c.online.lock],
    ["WalkIn_Rai", c.walkin.rai],
    ["WalkIn_Lock", c.walkin.lock],
    ["Cancel_Rai", c.cancel.rai],
    ["Cancel_Lock", c.cancel.lock],
    ["Absent_Rai", c.absent.rai],
    ["Absent_Lock", c.absent.lock],
  ];
  var cDate = col["วันที่"];
  var cL1 = findCol_(col, /^L1_/),
    cL2 = findCol_(col, /^L2_/);
  var missing = owned
    .filter(function (o) {
      return !col[o[0]];
    })
  ...
  owned.push(["__L1", c.elec]);
  owned.push(["__L2", c.tool]);
  var colOf = function (key) {
    return key === "__L1" ? cL1 : key === "__L2" ? cL2 : col[key];
  };
  ...
  owned.forEach(function (o) {
    sh.getRange(found, colOf(o[0])).setValue(o[1]);
  });
  Logger.log('🔄 "' + label + '" อัปเดตแถว ' + found + " (" + dateStr + ")");
  Logger.log(
    "     Online ล็อก " +
      before[col["Online_Lock"] - 1] +
      " → " +
      c.online.lock +
      " · WalkIn " +
      before[col["WalkIn_Lock"] - 1] +
      " → " +
      c.walkin.lock +
      " · ไม่มา " +
      before[col["Cancel_Lock"] - 1] +
      " → " +
      c.cancel.lock,
  );
...
function verify_(rows, wantDate, cfg) {
  var H = rows[0],
    data = rows.slice(1);
  var iDate = H.indexOf("วันที่ขาย"),
    iLock = H.indexOf("ล็อค");
  var iElec = H.indexOf("ค่าไฟฟ้า"),
    iTool = H.indexOf("ค่าอุปกรณ์");
  ...
  data.forEach(function (r) {
    var d = String(r[iDate] || "").slice(0, 10);
  ...
  Logger.log(
    "วันที่ขายในไฟล์: " +
      keys
        .map(function (k) {
          return k + " (" + dates[k] + " แถว)";
        })
        .join(" · "),
  );
  if (keys.length !== 1 || keys[0] !== wantDate) {
    Logger.log(
      "🔴 หยุด — ขอวันที่ " + wantDate + " แต่ไฟล์เป็น " + keys.join(","),
    );
  ...
  var stat = {};
  data.forEach(function (r) {
    var st = String(r[iStat] || "(ว่าง)");
    stat[st] = (stat[st] || 0) + 1;
    var codes = String(r[iLock] || "")
      .split(":")
      .filter(function (x) {
        return x.trim();
      });
    ...
    b.elec += num_(r[iElec]);
    b.tool += num_(r[iTool]);
    if (st === "จองหน้าเคาท์เตอร์") {
      b.walkin.rai += 1;
      b.walkin.lock += n;
    } else if (st === "ไม่มาขาย") {
      b.cancel.rai += 1;
      b.cancel.lock += n;
    } else if (st === "ลาคืนล็อค") {
      b.absent.rai += 1;
      b.absent.lock += n;
    }
  });
  ...
  ["st", "car"].forEach(function (k) {
    z[k].online = {
      rai: z[k].total.rai - z[k].walkin.rai,
...
function parseXlsx_(blob) {
  ...
  parts.forEach(function (f) {
    ...
    else if (n.indexOf("worksheets/sheet1.xml") !== -1)
      sheet = f.getDataAsString();
  });
  if (!sheet) return [];
  ...
  while ((rm = rowRe.exec(sheet)) !== null) {
    ...
    while ((cm = cRe.exec(rm[1])) !== null) {
      ...
      if (val === "" && /t="inlineStr"/.test(attr))
        val = unesc_((body.match(/<t[^>]*>([\s\S]*?)<\/t>/) || [])[1] || "");
...
function assertSafe_(payload, step) {
  ...
  var stop = function (why) {
    throw new Error("🔴 หยุด [" + step + "]: " + why + " — ไม่ส่งคำขอนี้");
  };

  Object.keys(payload).forEach(function (k) {
    // 1) blacklist เดิม — ชื่อปุ่มคืนเงินที่รู้จัก
    FORBIDDEN.forEach(function (f) {
      if (k.indexOf(f) !== -1) stop('มีปุ่มคืนเงิน "' + k + '"');
    });
    ...
    FORBIDDEN.forEach(function (f) {
      if (v.indexOf(f) !== -1) stop('ค่าของ "' + k + '" อ้างถึง ' + f);
    });
  ...
  var btn = allow.filter(function (k) {
    return k.indexOf("Button") !== -1 && k in payload;
  });
  if (btn.length !== 1)
    stop("ต้องกดปุ่มเดียวพอดี แต่เจอ " + btn.length + " ปุ่ม");
}
...
function pageControls_(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    var type = (
      tagAttr_(tag, "type") || (kind === "input" ? "text" : kind)
    ).toLowerCase();
    ...
    lines[
      [
        kind,
        type,
        norm(tagAttr_(tag, "name")),
        norm(tagAttr_(tag, "id")),
        isBtn ? tagAttr_(tag, "value") : "",
      ].join(" | ")
    ] = 1;
  }
  // ปลายทาง postback จากลิงก์/ปุ่มที่ใช้ JavaScript — ทางลัดไปกดปุ่มได้โดยไม่ผ่าน submit
  var pb =
    /(?:__doPostBack\(|WebForm_PostBackOptions\()\s*(?:&#39;|&quot;|['"])([^'"&]+)/g;
...
function requireButtons_(controls, where) {
  Object.keys(REQUIRED_BUTTONS).forEach(function (name) {
    ...
    var hit = controls.filter(function (l) {
      return l.split(" | ")[2] === name;
    });
    if (hit.length !== 1 || hit[0].split(" | ")[4] !== want) {
      throw new Error(
        "🔴 หยุด: " +
          ...
          " — ปุ่ม " +
          name +
          ' หายหรือข้อความเปลี่ยน (ต้องเป็น "' +
          want +
          '")',
      );
    }
...
function checkPage_(branch, stage, html) {
  ...
  if (!saved) {
    throw new Error(
      "🔴 หยุด: ยังไม่เคยอนุมัติหน้าตาของ" +
        where +
        " — รัน approvePage" +
        branch +
        " ก่อน",
  ...
  var added = now.filter(function (l) {
    return saved.indexOf(l) === -1;
  });
  var gone = saved.filter(function (l) {
    return now.indexOf(l) === -1;
  });
  if (added.length || gone.length) {
    ...
    throw new Error(
      "🔴 หยุด: " +
        ...
        " −" +
        gone.length +
        ") — ให้คนตรวจหน้าเว็บก่อน แล้วค่อยรัน approvePage" +
        branch +
        " ใหม่",
    );
  }
...
function approvePageBG() {
  approvePage_("BG");
}
function approvePageBN() {
  approvePage_("BN");
}
...
function approvePage_(branch) {
  ...
  requireButtons_(
    pageControls_(r1.getContentText()),
    "หน้ารายงาน " + branch + " ก่อนอนุมัติ",
  );
  var p2 = readHidden_(r1.getContentText());
  p2[F.start] = beStr;
  p2[F.effect] = beStr;
  ...
  [r1, r2].forEach(function (res, i) {
    var stage = i + 1,
      list = pageControls_(res.getContentText());
    Logger.log(
      "\n════════ " +
        branch +
        " ขั้น " +
        stage +
        (stage === 1 ? " (เปิดหน้า)" : " (หลังกดค้นหา)") +
        " · " +
        list.length +
        " รายการ ════════",
    );
  ...
  Logger.log(
    "\n✅ อนุมัติหน้าตาหน้าเว็บ " +
      branch +
      " แล้ว — ตัวดึงข้อมูลจะหยุดเองถ้าหน้าตาเปลี่ยนจากนี้",
  );
}

function login_(cfg, user, pass) {
  var SIGNIN = cfg.base + "Signin.aspx";
  var r1 = UrlFetchApp.fetch(SIGNIN, {
    muteHttpExceptions: true,
  ...
  if (r2.getResponseCode() !== 302 && r2.getResponseCode() !== 301) {
    Logger.log(
      "❌ login ไม่ผ่าน (HTTP " +
        r2.getResponseCode() +
        ") — เช็ค user/pass ใน Script Properties",
    );
    return "";
  }
...
function fmtCE_(d) {
  return (
    two_(d.getDate()) + "/" + two_(d.getMonth() + 1) + "/" + d.getFullYear()
  );
}
...
function readHidden_(html) {
  ...
  [
    ...
    var m =
      html.match(new RegExp('name="' + n + '"[^>]*value="([^"]*)"', "i")) ||
